...
## 🖥 میزبانی و مهاجرت به VPS

اپ در حال حاضر روی **Render (پلن رایگان، منطقه ohio)** و دیتابیس روی **Neon (us-east-1)** است.

اگر قصد مهاجرت به سرور مجازی را داری، قبل از هر کاری این دو فایل را بخوان:

| فایل | محتوا |
|---|---|
| [`docs/vps-onboarding.md`](docs/vps-onboarding.md) | چک‌لیست امنیتی قبل از دادن دسترسی، تصمیم دیتابیس، و کارهای باقی‌مانده |
| [`scripts/vps-provision.sh`](scripts/vps-provision.sh) | نصب خودکار Node/nginx/SSL/systemd/فایروال/بکاپ |

**نکات کلیدی که در آن سند مستند شده‌اند:**

- سرور **ایران** نگیر — تلگرام فیلتر است و Groq/OpenRouter/Resend/Supercell تحریم؛
  چهار قابلیت اصلی اپ از کار می‌افتد. سرور اروپا (آلمان) مشکلی ندارد.
- دیتابیس را **تقسیم نکن** بین دو سرویس؛ تراکنش‌های مالی اتمیک هستند و می‌شکنند.
- هنگام مهاجرت این‌ها فراموش می‌شوند: تغییر webhook تلگرام، rebuild لازم برای
  متغیرهای `NEXT_PUBLIC_*`، و رکورد DNS که باید قبل از certbot آماده باشد.
...
<!-- auto-deploy فعال: 2026-08-28 -->
<!-- دیپلوی خودکار فعال ✅ (تست از راه دور، 2026-08-28) -->
