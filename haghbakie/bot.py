...
logger.info(f"TOKEN={'✅' if BOT_TOKEN else '❌'} DB={'✅' if DATABASE_URL else '❌'} GROQ={'✅' if GROQ_API_KEY else '❌'}")
...
# ===================== حافظه موقت پرونده‌های در حال ثبت =====================
# {user_id: {title, content, side_a, side_b, verdict}}
pending_stories = {}
...
def get_db_conn():      return init_db_pool().getconn()
def return_db_conn(c):
    if db_pool and c:   db_pool.putconn(c)

class DBCursor:
    def __init__(self, conn): self.conn = conn
    def __enter__(self): self.cur = self.conn.cursor(cursor_factory=RealDictCursor); return self.cur
...
def init_db():
    ...
    try:
        ...
        logger.error(f"❌ DB: {e}"); conn.rollback(); raise
...
def ensure_user(user):
    ...
    try:
        ...
        logger.error(f"ensure_user: {e}"); conn.rollback()
    finally: return_db_conn(conn)
...
# ===================== تابع مشترک ثبت نهایی =====================
async def _do_confirm_story(user_id, username_hint="HaghBaKieBot", title="داستان جدید",
                             content="", side_a="من", side_b="طرف مقابل", verdict="⚖️ بدون نظر"):
    """ثبت پرونده در دیتابیس - بدون dependency به context"""
    conn = get_db_conn()
    try:
        with DBCursor(conn) as cur:
            cur.execute(
            ...
            sid = cur.fetchone()['id']
        conn.commit()
        logger.info(f"✅ Story #{sid} created by {user_id}")
        return sid, None
    except psycopg2.errors.ForeignKeyViolation:
        conn.rollback()
        logger.warning(f"FK violation for {user_id}, retrying...")
        # یک بار تلاش مجدد - کاربر رو توی دیتابیس ثبت کن
        try:
            with DBCursor(conn) as cur:
                cur.execute("""INSERT INTO hbk_users (user_id) VALUES (%s) ON CONFLICT DO NOTHING""", (user_id,))
            conn.commit()
            with DBCursor(conn) as cur:
                cur.execute(
                    """INSERT INTO hbk_stories (creator_id, title, content, side_a, side_b, ai_verdict, expires_at)
...
async def hot(update, context):
    ...
    if not stories: await update.message.reply_text("🔍 هنوز پرونده‌ای ثبت نشده!", reply_markup=main_kb()); return
...
async def lb(update, context):
    ...
    if not users: await update.message.reply_text("🏆 هنوز کسی امتیازی کسب نکرده.", reply_markup=main_kb()); return
...
async def my(update, context):
    ...
    if not stories: await update.message.reply_text("📭 پرونده‌ای ثبت نکرده‌اید.", reply_markup=main_kb()); return
    lines = ["📊 **پرونده‌های من:**\n"]
    for s in stories:
...
async def sub_start(update, context):
    ...
    pending_stories.pop(uid, None)  # پاک کردن داده قبلی
...
async def sub_content(update, context):
    ...
    if len(txt) < 20:
        ...
        return SUBMIT_STORY
    # ذخیره هم در user_data و هم در pending_stories
    context.user_data['c'] = txt
    pending_stories[uid] = {'c': txt}
...
async def sub_a(update, context):
    uid = update.effective_user.id
    txt = update.message.text.strip()
    context.user_data['a'] = txt
    if uid in pending_stories:
        pending_stories[uid]['a'] = txt
...
async def sub_b(update, context):
    uid = update.effective_user.id
    txt = update.message.text.strip()
    context.user_data['b'] = txt

    content = context.user_data.get('c', '')
    side_a = context.user_data.get('a', 'من')
    side_b = txt

    # ذخیره کامل در pending_stories (حافظه پشتیبان)
    pending_stories[uid] = {'c': content, 'a': side_a, 'b': side_b}

    w = await update.message.reply_text("🤖 در حال تحلیل با هوش مصنوعی...")
    title, verdict = ai_analyze(content, side_a, side_b)

    # ذخیره در هر دو جا
    context.user_data['ai'] = {'title': title, 'verdict': verdict}
    pending_stories[uid]['ai'] = {'title': title, 'verdict': verdict}

    await w.delete()
    preview = (f"🔖 **{title}**\n\n📝 {content}\n\n"
               f"👤 {side_a} 🆚 {side_b}\n\n🤖 {verdict}\n\n✅ ثبت شود؟")
    kb = [[InlineKeyboardButton("✅ تایید و انتشار", callback_data="confirm_story"),
...
# ⚡ این تابع هم از طریق ConversationHandler و هم به صورت گلوبال قابل دسترسی است
async def sub_confirm(update, context):
    q = update.callback_query
    await q.answer()

    uid = update.effective_user.id
    logger.info(f"🔔 Confirm callback from user {uid}, data={q.data}")

    if q.data == "cancel_story":
        await q.edit_message_text("❌ ثبت پرونده لغو شد.")
        pending_stories.pop(uid, None)
        context.user_data.clear()
        try: await update.effective_chat.send_message("📋 به منوی اصلی برگشتید.", reply_markup=main_kb())
        except: pass
        return ConversationHandler.END

    if q.data != "confirm_story":
        await q.answer("⚠️ خطای ناشناخته!", show_alert=True)
        return ConversationHandler.END

    # ======== تایید نهایی ========

    # گرفتن داده از pending_stories (حافظه پشتیبان) یا context.user_data
    ps = pending_stories.get(uid, {})
    ai_data = ps.get('ai') or context.user_data.get('ai', {})
    title = ai_data.get('title', 'داستان جدید')
    verdict = ai_data.get('verdict', '⚖️ بدون نظر')
    content = ps.get('c') or context.user_data.get('c', '')
    side_a = ps.get('a') or context.user_data.get('a', 'من')
    side_b = ps.get('b') or context.user_data.get('b', 'طرف مقابل')

    logger.info(f"📝 Confirm: uid={uid} title={title} content_len={len(content)}")

    if not content:
        await q.edit_message_text(
            "❌ **خطا: اطلاعات پرونده یافت نشد.**\n\n"
            "احتمالاً session بات منقضی شده. لطفاً:\n"
            "1️⃣ /start را بزنید\n"
            "2️⃣ دوباره روی «ثبت پرونده جدید» کلیک کنید",
    ...
    # ثبت پرونده
    sid, error = await _do_confirm_story(
        user_id=uid,
        title=title,
        content=content,
        side_a=side_a,
    ...
    if error:
        logger.error(f"❌ Confirm error: {error}")
        await q.edit_message_text(
            f"❌ **خطا در ثبت پرونده:**\n{error}\n\n"
    ...
    # موفقیت 🎉
    bu = "HaghBaKieBot"
    try: bu = (await context.bot.get_me()).username
    except: pass
    ...
    await q.edit_message_text(
        ...
        f"⏰ ۴۸ ساعت برای رأی‌گیری باز است.\n"
    ...
    try:
        if update and update.effective_chat:
            await context.bot.send_message(
                ...
                text="❌ خطایی رخ داد. لطفاً /start را بزنید."
            )
    except: pass
...
async def webhook_handler(request):
    try:
        ...
        # لاگ نوع آپدیت
        if update.callback_query:
            logger.info(f"📨 Callback: user={update.effective_user.id} data={update.callback_query.data}")
...
async def on_startup(app):
    ...
    # ⚡ sub_confirm به عنوان handler گلوبال (پشتیبان)
    # اگر ConversationHandler state رو گم کنه، این handler کار رو نجات میده
    ptb_app.add_handler(CallbackQueryHandler(sub_confirm, pattern="^(confirm|cancel)_story$"))

    # Conversation ثبت پرونده
    ptb_app.add_handler(ConversationHandler(
        entry_points=[MessageHandler(filters.Regex("^⚖️ ثبت پرونده جدید$"), sub_start)],
        states={
            SUBMIT_STORY: [MessageHandler(filters.TEXT & ~filters.COMMAND, sub_content)],
...
def main():
    logger.info("🚀 HaghBaKie starting...")
    try: init_db()
    except Exception as e: logger.critical(f"💥 DB: {e}"); sys.exit(1)
