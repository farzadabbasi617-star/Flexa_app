...
vi.mock("@/lib/zarinpal", async () => {
  const actual = await vi.importActual<typeof import("@/lib/zarinpal")>("@/lib/zarinpal");
...
function setLive() {
  ...
  process.env.PAYMENT_CALLBACK_BASE_URL = "https://www.gament1.ir";
  process.env.ZARINPAL_LIVE = "true";
}

describe("startZarinpalDeposit", () => {
  ...
  it("returns the payment url and never sends a query-string callback", async () => {
    ...
    requestPaymentMock.mockResolvedValue({
      ...
      paymentUrl: "https://payment.zarinpal.com/pg/StartPay/A00000000000000000000000000000123456",
    });
    ...
    expect(sent.callbackUrl).toBe("https://www.gament1.ir/api/wallet/deposit/zarinpal/callback");
