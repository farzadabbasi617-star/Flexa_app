...
describe("calculateDynamicTournamentPrizePool", () => {
  ...
  describe("head-to-head duels (Clash Royale 1V1)", () => {
    ...
    it("advertises the real 80,000 payout before anyone joins", () => {
      ...
      // Previously "۰ تومان (طبق ثبت‌نام)" with a 40,000,000 projection.
      expect(result.displayPrizePool).toBe("۸۰٬۰۰۰ تومان");
      expect(result.displayPrizePool).not.toContain("۰ تومان (طبق ثبت‌نام)");
    });
