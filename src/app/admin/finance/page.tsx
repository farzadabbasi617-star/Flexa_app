...
export default function AdminFinancePage() {
  ...
  const load = useCallback(async () => {
    setBusy(true);
    try { const res = await fetch("/api/admin/finance", { cache: "no-store" }); const json = await res.json(); setData(res.ok ? json : null); }
  ...
  return (
    <div className="min-h-screen bg-dark-900 text-white"><Navbar /><main className="max-w-7xl mx-auto px-4 sm:px-6 py-8">
      ...
      <div className="flex justify-between gap-4 mb-6"><div><h1 className="text-3xl font-black neon-text-purple">📈 گزارش مالی</h1><p className="text-gray-500 text-sm mt-2">تراکنش‌ها، موجودی‌ها، درآمدهای ورودی و خروجی‌ها</p></div><button onClick={exportCsv} className="gaming-btn text-sm">خروجی CSV</button></div>
