...
| `0048_add_tournament_tickets.sql` | Adds `tournament_tickets`: admin-issued free-entry tickets for paid tournaments. A ticket belongs to one user, optionally locked to one tournament (`tournament_id NULL` = any paid tournament), covers 100% of the entry fee, single-use by default (`max_uses`), with revoke/expiry tracking and the used tournament/registration recorded for audit. |
